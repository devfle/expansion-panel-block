...
for ( const EXPANSION_PANEL of EXPANSION_PANELS ) {
	...
	const PANEL_HEIGHT = EXPANSION_PANEL_CONTENT.offsetHeight;
	...
	EXPANSION_PANEL.addEventListener( 'click', () => {
		...
		if (
			EXPANSION_PANEL.classList.contains( 'devfle-expansion-panel--open' )
		) {
			EXPANSION_PANEL_CONTENT.style.height = `${
				PANEL_HEIGHT + DEFAULT_PADDING * 2
			}px`;
		} else {
			EXPANSION_PANEL_CONTENT.style.height = 0;
		}
